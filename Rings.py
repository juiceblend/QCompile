...
class Exact_U: 
    ...
    def __mul__(self, A):
        return Exact_U(self.u*A.u + self.v.star()*A.v*self.tau*(self.omega**self.k), 
                 self.v*A.u - self.u.star()*A.v*(self.omega**self.k), 
                 (self.k + A.k + 5)%10)
    ...
    # faster version of mu(F*T**j*self)
    def j_mu(self, j):
        upper_left = self.tau * (self.u + self.v * self.omega**j)
        return upper_left.mu()
